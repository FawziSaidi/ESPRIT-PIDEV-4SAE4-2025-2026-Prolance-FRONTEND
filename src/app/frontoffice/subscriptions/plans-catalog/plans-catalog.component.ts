...
export class PlansCatalogComponent implements OnInit, OnDestroy {
  ...
  selectedType: string = 'freelancer';
  ...
  private roleSub: RxSubscription | null = null;
  ...
  loadPlans(): void {
    ...
    const typeForApi = this.selectedType.toUpperCase();
    
    this.subscriptionService.getSubscriptionsByType(typeForApi).subscribe(
      (data: Subscription[]) => {
  ...
  filterPlans(): void {
    this.filteredPlans = this.plans.filter(
      (plan) => plan.billingCycle === this.billingCycle
    );
  }
  ...
  getFeatures(plan: Subscription): string[] {
    ...
    if (this.selectedType === 'client') {
      if (plan.maxProjects) {
        features.push(plan.maxProjects >= 999 ? 'Offres illimitées' : `${plan.maxProjects} offres d'emploi`);
      }
      if (plan.maxProposals) {
        features.push(plan.maxProposals >= 999 ? 'Propositions illimitées' : `${plan.maxProposals} propositions`);
      }
      if (plan.featuredListing) features.push('Matching IA');
      if (plan.maxActiveJobs) {
        features.push(plan.maxActiveJobs >= 999 ? 'Équipe multi-users' : `${plan.maxActiveJobs} recrutements`);
      }
      ...
      if (plan.maxProjects) {
        features.push(plan.maxProjects >= 999 ? 'Projets illimités' : `${plan.maxProjects} projets max`);
      }
  ...
  onModalConfirmed(event: { plan: Subscription; autoRenew: boolean }): void {
    ...
    this.subscriptionService.subscribe(event.plan.id!, 'CARD').subscribe(
      (userSub) => {
        alert('✅ Abonnement réussi ! Bienvenue dans le plan ' + event.plan.name);
      },
      (error) => {
        console.error('Erreur abonnement:', error);
        alert('❌ Erreur : ' + (error.error?.message || 'Impossible de souscrire. Réessayez.'));
      }
    );
  }
